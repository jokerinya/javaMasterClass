...
public class Main {
    ...
    public static boolean isPalindrome(int number){
        if(number<0){
            number *= -1;  // makes negative to positive
        }
        int controlNumber = number;  // secure number for later condition
        int reverseNumber = 0;  // assume reversed number as 0
        while (number>0){
            int lastDigit = number % 10; // get last digit
            reverseNumber = reverseNumber * 10 + lastDigit;  // add last digit to rev num at every loop after * 10
            number /= 10;  // discard first from right digit
        }
        return controlNumber==reverseNumber;
    }
}
